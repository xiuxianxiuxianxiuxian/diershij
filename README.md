...
启动 PostgreSQL、Redis 和全部 5 个微服务。
...
CLI 客户端支持 21+ 个命令：`cult`（修炼）、`bt`（突破）、`exp`（探索）、`combat`（战斗）、`skill`（技能）、`flee`（逃跑）、`spell`（法术）、`move`（移动）、`gather`（采集）、`craft`（制作）、`cm`（创造功法）、`chat`（聊天）、`msg`（消息）、`friend`（好友）、`trade`（交易）、`st`（状态）、`create_sect`（创建宗门）、`join_sect`（加入）、`leave_sect`（离开）。
...
- [x] 邮件系统 — 离线消息、系统通知
- [x] 排行榜 — 境界/战力全服排行
- [x] 世界事件系统 — 妖兽潮、天道异象、灵潮、秘境开启、宗门战
- [x] Bubble Tea TUI 客户端 — 终端交互界面
- [ ] 跨服组队副本
- [ ] 宗门战/领地争夺

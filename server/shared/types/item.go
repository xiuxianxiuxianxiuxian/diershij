...
// InventoryItem represents a single item in an entity's inventory.
type InventoryItem struct {
	TemplateID string `json:"template_id"`
	InstanceID string `json:"instance_id"`  // unique instance ID
	Name       string `json:"name"`
	Quantity   int    `json:"quantity"`
	Slot       int    `json:"slot"`         // inventory slot position
	Quality    int    `json:"quality"`      // 1-100 (randomized quality)
	Bound      bool   `json:"bound"`        // whether bound to entity
	ExpiryTime int64  `json:"expiry_time"`  // expiry timestamp (0 = permanent)
}

...
1. **NPC数量规模**：单个服务器预期支持多少玩家和NPC同时在线？（影响AI计算资源规划和服务器架构设计）
2. **LLM集成方式**：使用自有部署的开源模型（如本地运行Qwen、LLaMA），还是调用第三方API（如OpenAI、Claude、智谱等）？
3. **初始世界设定**：新世界初始化时需要哪些基础设定？（地图、资源分布、初始NPC数量等）

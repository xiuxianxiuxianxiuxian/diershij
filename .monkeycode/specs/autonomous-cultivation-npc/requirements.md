...
| LLM集成方式 | 方案A：调用第三方API（OpenAI/Claude/智谱等） |
| 世界初始化 | 新服务器启动时预置基础内容（地图、资源分布、引导NPC） |

## Pending Clarifications

以下问题需要与用户进一步确认以完善需求：

1. **预置内容范围**：初始世界需要预置哪些具体元素？（如基础地图结构、灵气节点分布、初始NPC数量和行为倾向、基础资源刷新规则等）
2. **API选择**：倾向于使用哪个LLM提供商？（OpenAI GPT-4o、Claude、智谱GLM、通义千问等，影响延迟和成本）
3. **世界尺寸**：初始世界包含多少个区域/地点？
